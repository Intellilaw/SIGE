...
async function assertSenderConnected(request: FastifyRequest, senderEmail: string) {
  ...
  if (!connection) {
    throw new AppError(
      409,
      "PERIODIC_MESSAGE_SENDER_NOT_CONNECTED",
      "El remitente debe conectar su cuenta de Google Workspace antes de activar la programación."
    );
  }
}
...
export const periodicMessagesRoutes: FastifyPluginAsync = async (app) => {
  app.get("/periodic-messages/config", { preHandler: [requireAuth] }, async (request) => {
    ...
    const [configuration, connectedSenders] = await Promise.all([
      ...
      prisma.googleWorkspaceConnection.count({
        where: { organizationId: user.organizationId, status: "ACTIVE", refreshTokenCiphertext: { not: null } }
      })
    ]);
    return {
      ...
      notice: configuration.configured
        ? "Google Workspace está listo para conectar remitentes. El worker de envíos programados permanece deshabilitado."
        : configuration.error
    };
  });
  ...
  app.get("/periodic-messages/senders", { preHandler: [requireAuth] }, async (request) => {
    ...
    return prisma.$queryRaw<Array<{
      ...
      SELECT u."id", u."email", u."displayName",
        COALESCE(gwc."status", 'NOT_CONNECTED') AS "connectionStatus",
        gwc."connectedAt" AS "connectedAt"
      FROM "User" u
      INNER JOIN "TaskModule" tm ON tm."id" = ${query.teamKey}
      LEFT JOIN "GoogleWorkspaceConnection" gwc
        ON gwc."organizationId" = u."organizationId"
        AND lower(gwc."email") = lower(u."email")
      WHERE u."organizationId" = ${user.organizationId}
        AND u."isActive" = true
        AND lower(u."email") LIKE '%@rusconi.law'
        AND (u."legacyTeam" = tm."team" OR u."secondaryLegacyTeam" = tm."team" OR u."team" = tm."team" OR u."secondaryTeam" = tm."team")
