...
import type { LegacyTaskTableConfig } from "./task-legacy-config";
...
export function usesOptionalTermToggle(table: LegacyTaskTableConfig | undefined) {
  return table?.slug === "escritos-fondo"
    || table?.slug === "escritos"
    || table?.slug === "oficios";
}

export function usesPresentationAndTermDates(table: LegacyTaskTableConfig | undefined) {
...
export function hasMeaningfulTaskLabel(value?: string | null) {
...
function getRecordDataText(record: TaskTrackingRecord, key: string) {
...
function getLegacyDataTaskName(record: TaskTrackingRecord) {
  return LEGACY_TASK_NAME_KEYS.map((key) => getRecordDataText(record, key)).find(hasMeaningfulTaskLabel) ?? "";
}

export function getTableTaskFallback(table?: LegacyTaskTableConfig) {
  return normalize(table?.title).replace(/^\d+\s*[\).-]?\s*/, "");
}

export function buildDistributionHistoryTaskNameMap(histories: TaskDistributionHistory[]) {
...
function historyMatterMatches(record: TaskTrackingRecord, history: TaskDistributionHistory) {
...
function targetTableMatches(record: TaskTrackingRecord, table: LegacyTaskTableConfig | undefined, targetTable: string) {
...
export function resolveHistoryTaskName(
...
export function resolveTrackingTaskName(
  record: TaskTrackingRecord,
  table?: LegacyTaskTableConfig,
