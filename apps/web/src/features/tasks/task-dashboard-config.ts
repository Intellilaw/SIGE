...
export interface TaskDashboardMember {
  id: string;
  name: string;
  aliases: string[];
}
...
interface TaskDashboardConfig {
  members: TaskDashboardMember[];
  sharedResponsibleAliases?: string[];
}

export const TASK_DASHBOARD_CONFIG_BY_MODULE_ID: Record<TaskModuleDefinition["id"], TaskDashboardConfig> = {
  litigation: {
    members: [
      {
        id: "MEOO",
