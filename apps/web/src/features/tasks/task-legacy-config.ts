...
export type LegacyTaskTableMode = "status" | "workflow";

export interface LegacyTaskTab {
  key: string;
  label: string;
  status?: "pendiente" | "presentado";
  stage?: number;
  isCompleted: boolean;
}

export interface LegacyTaskTableConfig {
  slug: string;
  sourceTable: string;
  title: string;
  mode: LegacyTaskTableMode;
  dateLabel: string;
  termDateLabel?: string;
  autoTerm?: boolean;
  termManagedDate?: boolean;
  dateEditable?: boolean;
  showDateColumn?: boolean;
  showReportedPeriod?: boolean;
  reportedPeriodLabel?: string;
  periodicProcess?: boolean;
  responsibleOptions?: string[];
  restrictResponsibleOptions?: boolean;
...
export interface LegacyTaskModuleConfig {
  ...
  termDateLabel: string;
  verificationColumns: Array<{ key: string; label: string }>;
  tables: LegacyTaskTableConfig[];
  hasRecurringTerms: boolean;
}

const statusTabs = (
  pendingLabel = "1. Pendientes",
  completedLabel = "2. Completados"
): LegacyTaskTab[] => [
...
const workflowTabs = (labels: string[]): LegacyTaskTab[] =>
...
const table = (config: Omit<LegacyTaskTableConfig, "tabs" | "mode" | "dateLabel"> & {
  tabs?: LegacyTaskTab[];
  mode?: LegacyTaskTableMode;
  dateLabel?: string;
}): LegacyTaskTableConfig => ({
...
const litigationTables: LegacyTaskTableConfig[] = [
  table({ slug: "escritos-fondo", sourceTable: "escritos_fondo", title: "1. Escritos de fondo", mode: "workflow", tabs: workflowTabs(["1. Pendientes", "2. Terminados sin presentar", "3. Presentados", "4. Dados de alta en BE y BL", "5. Auto de radicación revisado"]), dateLabel: "Fecha debe presentarse", responsibleOptions: ["MEOO", "EKPO", "NBSG"], restrictResponsibleOptions: true }),
...
const corporateTables: LegacyTaskTableConfig[] = [
...
const conveniosTables: LegacyTaskTableConfig[] = [
...
const financieroTables: LegacyTaskTableConfig[] = [
...
const complianceTables: LegacyTaskTableConfig[] = [
...
export const LEGACY_TASK_MODULES: LegacyTaskModuleConfig[] = [
  {
    slug: "litigio",
    moduleId: "litigation",
    team: "LITIGATION",
...
export const LEGACY_TASK_MODULE_BY_SLUG = Object.fromEntries(
  LEGACY_TASK_MODULES.map((module) => [module.slug, module])
) as Record<string, LegacyTaskModuleConfig>;
...
export function getAdjacentLegacyTaskTable(module: LegacyTaskModuleConfig, tableSlug: string, direction: -1 | 1) {
