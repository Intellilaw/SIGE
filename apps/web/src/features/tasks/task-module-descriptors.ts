...
import { EXECUTION_MODULE_BY_ID, type ExecutionModuleDescriptor } from "../execution/execution-config";
...
export interface TaskModuleDescriptor {
  ...
  legacyExecutionModule?: ExecutionModuleDescriptor;
}
...
export function buildTaskModuleDescriptors(modules: TaskModuleDefinition[]) {
  return modules.map(buildTaskModuleDescriptor);
}

export function findTaskModuleDescriptorBySlug(modules: TaskModuleDefinition[], slug?: string) {
  if (!slug) {
    return undefined;
  }
...
export function buildTaskDashboardMembers(module: TaskModuleDefinition): TaskDashboardMember[] {
