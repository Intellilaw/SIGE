...
function isPayrollPercentageField(field) {
...
function getPayrollGrossCompensationMxn(entry) {
    const total = (Number(entry.netSalaryMxn || 0) +
...
export function GeneralExpensesPage() {
    ...
    const payrollDistributionAutosaveTimersRef = useRef({});
    ...
    async function loadPayrollEntries({ showLoading = true, clearDrafts = true } = {}) {
        if (!canRead) {
            setPayrollEntries([]);
    ...
    function cancelPayrollDistributionAutosave(entryId) {
    ...
    function getPayrollDistributionPatch(entry, overrides = {}) {
        const entryDrafts = payrollDrafts[entry.id] ?? {};
        const getValue = (field) => clampPercentage(Number(overrides[field] ?? entryDrafts[field] ?? entry[field] ?? 0));
        return {
    ...
    function clearPayrollDistributionDrafts(entryId) {
    ...
    async function persistPayrollDistribution(entryId, patch) {
        ...
        await enqueuePayrollPatch(entryId, async () => {
            try {
                const updated = await apiPatch(`/general-expenses/payroll/${entryId}/distribution`, patch);
                if (payrollPatchSequenceRef.current[entryId] !== requestSequence) {
                    return;
                }
    ...
    async function flushPayrollDistribution(entry, overrides = {}) {
    ...
    function schedulePayrollDistributionAutosave(entry, field, rawValue) {
    ...
    function handlePayrollGeneralExpenseChange(entry, generalExpense) {
        const localPatch = generalExpense
            ? {
                generalExpense,
                pctLitigation: 20,
                pctCorporateLabor: 20,
    ...
    function renderPayrollTable(half, title) {
        ...
        return (_jsxs("section", { className: "panel", children: [_jsxs("div", { className: "panel-header", children: [_jsx("h2", { children: title }), _jsx("button", { type: "button", className: "primary-button", onClick: () => void handleAddPayrollEntry(half), disabled: !canWrite, children: "+ Agregar fila" })] }), _jsx("div", { className: "lead-table-shell", children: _jsx("div", { className: "lead-table-wrapper general-expense-payroll-table-wrapper", children: _jsxs("table", { className: "lead-table general-expense-payroll-table", children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { className: "general-expense-payroll-employee-column", children: "Nombre del colaborador" }), _jsx("th", { children: "Medio tiempo" }), _jsx("th", { children: "Salario diario" }), _jsx("th", { children: "Salario bruto" }), _jsx("th", { children: "Faltas" }), _jsx("th", { children: "Salario menos faltas" }), _jsx("th", { children: "D\u00EDas disfrutados por adelantado" }), _jsx("th", { children: "Fecha en la que se pagar\u00E1 la prima vacacional" }), _jsx("th", { children: "Pagar esta quincena" }), _jsx("th", { children: "D\u00EDas pagados correspondientes a quincenas anteriores" }), _jsx("th", { children: "D\u00EDas pagados correspondientes a esta quincena" }), _jsx("th", { children: "Total de d\u00EDas cuya prima vacacional es pagada esta quincena" }), _jsx("th", { children: "Prima vacacional" }), _jsx("th", { children: "Bono de puntualidad" }), _jsx("th", { children: "Bono de asistencia" }), _jsx("th", { children: "Horas extras (valor)" }), _jsx("th", { children: "N\u00FAmero de horas extras" }), _jsx("th", { children: "Total horas extras" }), _jsx("th", { children: "Detalle de horas extras" }), _jsx("th", { children: "Remuneraci\u00F3n total bruta" }), _jsx("th", { children: "Gasto general" }), DISTRIBUTION_FIELDS.map((field) => (_jsx("th", { children: field.label }, `payroll-${field.key}`))), _jsx("th", { children: "SUM %" }), _jsx("th", { children: "Retenciones ISR" }), _jsx("th", { children: "Retenciones IMSS" }), _jsx("th", { children: "Subsidio al empleo" }), _jsx("th", { children: "Credito Infonavit" }), _jsx("th", { children: "Dep\u00F3sito neto" }), _jsx("th", { children: "Confirmo que la n\u00F3mina est\u00E1 timbrada (ALE)" }), _jsx("th", { children: "Pago autorizado EMRT" }), _jsx("th", { children: "Aprobado por JNLS" }), _jsx("th", { children: "Acciones" })] }) }), _jsx("tbody", { children: loadingPayroll ? (_jsx("tr", { children: _jsx("td", { colSpan: 36, className: "centered-inline-message", children: "Cargando n\u00F3mina..." }) })) : rows.length === 0 ? (_jsx("tr", { children: _jsx("td", { colSpan: 36, className: "centered-inline-message", children: "Sin registros de n\u00F3mina en esta quincena." }) })) : (_jsxs(_Fragment, { children: [rows.map((entry) => {
                                                        ...
                                                        return records.map((expense, index) => {
                                                            ...
                                                            return (_jsxs("tr", { ref: (node) => {
                                                                    ...
                                                                    sourceManaged ? "general-expense-row-source-managed" : ""
                                                                                ...
                                                                                }, disabled: emrtApprovalDisabled, "aria-label": "Aprobado por EMRT" }) }) }), _jsx("td", { children: expense.paymentMethod === "Efectivo" && expense.approvedByEmrt ? (_jsxs("div", { className: "general-expense-date-stack", children: [_jsx("input", { className: "general-expense-input", type: "date", value: toDateInput(expense.paidByEmrtAt), onChange: (event) => handlePaidByEmrtDateChange(expense, event.target.value || null), disabled: !canEditEmrtDate || preEmrtLocked }), _jsx("button", { type: "button", className: "general-expense-inline-button", onClick: () => handlePaidByEmrtDateChange(expense, getTodayInput()), disabled: !canEditEmrtDate || preEmrtLocked, children: "Hoy" })] })) : null }), _jsx("td", { className: "general-expense-checkbox-cell general-expense-emrt-reimbursement-column", children: _jsx("input", { type: "checkbox", checked: expense.paymentMethod === "Transferencia" && expense.emrtReimbursementPending, onChange: (event) => void persistExpensePatch(expense.id, { emrtReimbursementPending: event.target.checked }), disabled: !canEditEmrtReimbursement || expense.paymentMethod !== "Transferencia" || sourceManaged }) }), _jsx("td", { className: "general-expense-checkbox-cell", children: _jsx("input", { type: "checkbox", checked: expense.reviewedByJnls, onChange: (event) => void persistExpensePatch(expense.id, { reviewedByJnls: event.target.checked }), disabled: !canReviewJnlsFlag }) }), _jsx("td", { className: "general-expense-checkbox-cell", children: _jsx("input", { type: "checkbox", checked: expense.paid, onChange: (event) => void persistExpensePatch(expense.id, { paid: event.target.checked }), disabled: !canPay || payrollManaged, title: payrollManaged ? "El pago de la Nomina se registra automaticamente." : undefined }) }), _jsx("td", { children: _jsx("input", { className: "general-expense-input", type: "date", value: toDateInput(expense.paidAt), onChange: (event) => void persistExpensePatch(expense.id, { paidAt: event.target.value || null }), disabled: !canWrite || payrollManaged, title: payrollManaged ? "Fecha calculada segun la quincena y los dias inhabiles generales." : undefined }) }), _jsx("td", { className: "general-expense-money-cell", children: distribution.withoutTeam > 0 ? formatCurrency(distribution.withoutTeam) : "-" }), _jsx("td", { className: "general-expense-money-cell is-accent", children: distribution.litigation > 0 ? formatCurrency(distribution.litigation) : "-" }), _jsx("td", { className: "general-expense-money-cell is-accent", children: distribution.corporateLabor > 0 ? formatCurrency(distribution.corporateLabor) : "-" }), _jsx("td", { className: "general-expense-money-cell is-accent", children: distribution.settlements > 0 ? formatCurrency(distribution.settlements) : "-" }), _jsx("td", { className: "general-expense-money-cell is-accent", children: distribution.financialLaw > 0 ? formatCurrency(distribution.financialLaw) : "-" }), _jsx("td", { className: "general-expense-money-cell is-accent", children: distribution.taxCompliance > 0 ? formatCurrency(distribution.taxCompliance) : "-" }), _jsx("td", { className: "general-expense-money-cell is-total", children: distribution.totalPaid > 0 ? formatCurrency(distribution.totalPaid) : "-" }), _jsx("td", { children: _jsx("button", { type: "button", className: "danger-button general-expense-delete-button", onClick: () => void handleDelete(expense), disabled: !canWrite || expense.approvedByEmrt || preEmrtLocked || sourceManaged, title: sourceManaged ? `Este gasto solo puede eliminarse desaprobando su fila en ${sourceModuleLabel}.` : undefined, children: "Borrar" }) })] }, expense.id));
                                                        });
