...
function normalizeVisibilityOptions(options) {
    const unique = new Set([GENERAL_VISIBILITY]);
    options
...
function findClientBySearchValue(clients, value) {
...
function findMatterClientMatch(clients, matter) {
    if (matter.clientId) {
...
function matchesClientSearch(clientName, searchWords) {
...
function matchesWordSearch(matter, quotes, clients, reflection, searchWords) {
    if (searchWords.length === 0) {
        return true;
    }
...
function MatterTable({ items, loading, quotes, clients, reflections, commissionOptions, visibilityOptions, selectedIds, rowRefs, readOnly, variant, canEditCommissionAssignee, canDeleteReadOnlyRows, onToggleSelection, onToggleAll, onClientChange, onLocalChange, onImmediateChange, onCommissionAssigneeChange, onQuoteChange, onBlur, onGenerateIdentifier, onSendToExecution, onTrash }) {
    ...
    return (_jsx("div", { className: "lead-table-shell", children: _jsxs("div", { className: "lead-table-wrapper", children: [_jsx("datalist", { id: clientSearchListId, children: clients.map((client) => (_jsx("option", { value: getClientSearchLabel(client) }, client.id))) }), _jsxs("table", { className: `lead-table matters-table ${isRetainerTable ? "matters-table-retainer" : "matters-table-unique"}`, children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { className: "lead-table-checkbox", children: _jsx("input", { type: "checkbox", checked: allSelected, disabled: readOnly, onChange: () => onToggleAll(items) }) }), _jsx("th", { children: "No. Cliente" }), _jsx("th", { children: "Cliente" }), _jsx("th", { children: "No. Cotizacion" }), _jsx("th", { children: "Tipo" }), _jsx("th", { children: "Asunto" }), !isRetainerTable ? _jsx("th", { children: "Proceso especifico" }) : null, _jsx("th", { children: "Total" }), _jsx("th", { children: "Comision cierre" }), !isRetainerTable ? _jsx("th", { children: "Canal" }) : null, !isRetainerTable ? _jsx("th", { children: "R1 Int" }) : null, !isRetainerTable ? _jsx("th", { children: "Bot TG" }) : null, !isRetainerTable ? _jsx("th", { children: "RD" }) : null, !isRetainerTable ? _jsx("th", { children: "RF" }) : null, !isRetainerTable ? _jsx("th", { children: "R1 Ext" }) : null, !isRetainerTable ? _jsx("th", { children: "Chat Fac" }) : null, !isRetainerTable ? _jsx("th", { children: "Notas" }) : null, !isRetainerTable ? _jsx("th", { children: "ID Asunto" }) : null, !isRetainerTable ? _jsx("th", { children: "Generar" }) : null, !isRetainerTable ? _jsx("th", { children: "Vinculado" }) : null, !isRetainerTable ? _jsx("th", { children: "Equipo" }) : null, !isRetainerTable ? _jsx("th", { children: "Enviar" }) : null, !isRetainerTable ? _jsx("th", { children: "Siguiente tarea" }) : null, !isRetainerTable ? _jsx("th", { children: "Fecha sig." }) : null, !isRetainerTable ? _jsx("th", { children: "Origen" }) : null, !isRetainerTable ? _jsx("th", { children: "Visibilidad" }) : null, !isRetainerTable ? _jsx("th", { children: "Hito conclusion" }) : null, !isRetainerTable ? _jsx("th", { children: "Concluyo?" }) : null, _jsx("th", { children: "Borrar" })] }) }), _jsx("tbody", { children: loading ? (_jsx("tr", { children: _jsx("td", { colSpan: isRetainerTable ? 9 : 29, className: "centered-inline-message", children: "Cargando asuntos..." }) })) : items.length === 0 ? (_jsx("tr", { children: _jsx("td", { colSpan: isRetainerTable ? 9 : 29, className: "centered-inline-message", children: "No hay asuntos." }) })) : (items.map((item) => {
                                ...
                                const quoteOptions = linkedQuote && !clientQuoteOptions.some((quote) => quote.id === linkedQuote.id)
                                    ...
                                    : clientQuoteOptions;
                                const hasSelectedQuoteOption = !normalizeText(item.quoteNumber) ||
...
export function MattersPage() {
    ...
    const [visibilityOptions, setVisibilityOptions] = useState([GENERAL_VISIBILITY]);
    ...
    const [pendingScrollMatterId, setPendingScrollMatterId] = useState(null);
    const matterRowRefs = useRef(new Map());
    ...
    async function loadBoard() {
        ...
        try {
            const [loadedMatters, loadedDeleted, loadedQuotes, loadedClients, loadedTaskItems, loadedTaskModules, shortNames, loadedVisibilityOptions] = await Promise.all([
                apiGet("/matters"),
                ...
                apiGet("/matters/visibility-options")
            ]);
    ...
    async function persistMatterPayload(matterId, payload) {
        try {
    ...
    function handleClientChange(matterId, value) {
        if (!canWriteMatters) {
            return;
        }
        updateMatterLocal(matterId, (matter) => {
    ...
    async function handleCommissionAssigneeChange(matterId, value) {
    ...
    const filteredDeletedItems = useMemo(() => deletedItems.filter((item) => {
        const teamMatches = teamFilter === "Todos" || item.responsibleTeam === teamFilter;
        const reflection = getMatterReflection(item, reflections);
        const clientMatches = matchesClientSearch(item.clientName, clientSearchWords);
        const wordMatches = matchesWordSearch(item, quotes, clients, reflection, wordSearchWords);
