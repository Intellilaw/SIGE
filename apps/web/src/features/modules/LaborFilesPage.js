...
export function LaborFilesPage() {
    ...
    const [vacationConflictCheck, setVacationConflictCheck] = useState(null);
    ...
    const [pendingVacationConflictRequests, setPendingVacationConflictRequests] = useState([]);
    const [loadingPendingVacationConflicts, setLoadingPendingVacationConflicts] = useState(false);
    const [pendingVacationConflictNotes, setPendingVacationConflictNotes] = useState({});
    const [authorizingPendingVacationConflictId, setAuthorizingPendingVacationConflictId] = useState(null);
    ...
    async function refreshPendingVacationConflictRequests(showError = false) {
        ...
        try {
            const requests = await apiGet("/labor-files/vacation-conflict-requests/pending");
    ...
    useEffect(() => {
        ...
        setVacationConflictAuthorizationNote("");
        void apiPost(`/labor-files/${selectedLaborFile.id}/vacation-format/conflicts/check`, { vacationDates: vacationFormatForm.vacationDates })
            .then((result) => {
            if (!cancelled) {
    ...
    async function refreshVacationConflictCheck(dates = vacationFormatForm.vacationDates) {
        ...
        setCheckingVacationConflicts(true);
        try {
            const result = await apiPost(`/labor-files/${selectedLaborFile.id}/vacation-format/conflicts/check`, { vacationDates: dates });
            setVacationConflictCheck(result);
            return result;
        }
        finally {
    ...
    async function handleVacationConflictAuthorize() {
        ...
        setFlash(null);
        try {
            const result = await apiPost(`/labor-files/${selectedLaborFile.id}/vacation-format/conflicts/authorize`, {
                vacationDates: vacationFormatForm.vacationDates,
                note: vacationConflictAuthorizationNote
            });
            setVacationConflictCheck(result);
            setPendingVacationConflictRequests((current) => current.filter((request) => request.laborFileId !== selectedLaborFile.id));
            setFlash({
                tone: "success",
    ...
    async function handlePendingVacationConflictAuthorize(request) {
        if (!canOverrideVacationConflicts) {
            return;
        }
        ...
        setFlash(null);
        try {
            const result = await apiPost(`/labor-files/${request.laborFileId}/vacation-format/conflicts/authorize`, {
            ...
            if (selectedLaborFile?.id === request.laborFileId &&
