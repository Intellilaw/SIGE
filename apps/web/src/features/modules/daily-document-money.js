export const moneyCurrencyLabels = {
    MXN: "Pesos (MXN)",
    USD: "Dólares (USD)"
};
const moneyCurrencyLegalDetails = {
...
const spanishNumberScales = [
    { value: 1000000000000000000000000n, singular: "cuatrillón", plural: "cuatrillones" },
    { value: 1000000000000000000n, singular: "trillón", plural: "trillones" },
    { value: 1000000000000n, singular: "billón", plural: "billones" },
    { value: 1000000n, singular: "millón", plural: "millones" }
];
function normalizeText(value) {
    return (value ?? "").trim();
}
function spanishNumberBelowOneThousand(number, apocope) {
    if (number < 30) {
        if (apocope && number === 1) {
            return "un";
        }
...
function integerToSpanish(number, apocope = false) {
    if (number < 1000n) {
    ...
    for (const scale of spanishNumberScales) {
        if (number >= scale.value) {
            ...
            const scaleText = scaleCount === 1n ? `un ${scale.singular}` : `${integerToSpanish(scaleCount, true)} ${scale.plural}`;
...
export function formatMoneyInputValue(rawAmount, forceDecimals = false) {
...
export function getMoneyCurrency(values) {
    return values.currency === "USD" ? "USD" : "MXN";
}
function moneyAmountParts(rawAmount) {
...
export function moneyAmountInWords(values) {
    const amountParts = moneyAmountParts(values.amount);
    if (!amountParts) {
    ...
    const requiresDe = amountParts.integer >= 1000000n && amountParts.integer % 1000000n === 0n;
    const words = integerToSpanish(amountParts.integer, true);
    return `${words}${requiresDe ? " de" : ""} ${currencyName} ${amountParts.decimals}/100 ${currencyDetails.abbreviation}`.toLocaleUpperCase("es-MX");
}
export function moneyAmountWithWords(values) {
