...
function writeWindowMessage(targetWindow: Window, title: string, message: string) {
  try {
...
export async function openBriefManagerWindow() {
  ...
  try {
    const response = await apiGet<BriefManagerSsoResponse>("/auth/sso/manager-de-escritos");
...
export function reportBriefManagerOpenError(reason: unknown) {
