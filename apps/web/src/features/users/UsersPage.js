...
function isEduardoRusconiSuperadmin(candidate) {
    if (!candidate) {
        return false;
    }
...
export function UsersPage() {
    ...
    async function handleTeamSave(event) {
        ...
        try {
            if (editingTeamId) {
                await apiPatch(`/users/teams/${editingTeamId}`, { label });
                setFlash({ tone: "success", text: "Equipo actualizado correctamente." });
            }
            else {
                await apiPost("/users/teams", { label });
