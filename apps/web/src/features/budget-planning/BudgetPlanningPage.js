...
function getExpectedIncomeDueToday(record) {
...
export function BudgetPlanningPage() {
    ...
    const totals = useMemo(() => {
        ...
        const expectedHighProbabilityIncomeMxn = financeRecords.reduce((sum, record) => sum + (record.highCollectionProbability ? getExpectedIncomeDueToday(record) : 0), 0);
        const expectedLowProbabilityIncomeMxn = financeRecords.reduce((sum, record) => sum + (record.lowCollectionProbability ? getExpectedIncomeDueToday(record) : 0), 0);
