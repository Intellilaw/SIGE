...
function TeamMessages({ teams, slug, onBack }) {
    ...
    async function load(moduleId) {
        const [senderRows, messageRows, signatureRows] = await Promise.all([
            ...
            apiGet(`/periodic-messages/signatures?teamKey=${encodeURIComponent(moduleId)}`)
        ]);
