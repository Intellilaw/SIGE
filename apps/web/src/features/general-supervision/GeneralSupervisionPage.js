...
function TaskOverviewPanel(props) {
    const { overview, kpiAlertsByUser, kpiWeekReference, kpiOverrides, kpiOverridePeriods, savingObservedUserId, savingKpiOverrideKey, onToggleObserved, onToggleKpiOverride } = props;
...
function KpiOverrideControls(props) {
    ...
    const dailyByDate = new Map([
        ...props.calendarPeriods.flatMap((period) => period.users
        ...
        ...props.periods.flatMap((period) => period.metrics
            .filter((metric) => metric.id === props.metric.id)
            .flatMap((metric) => metric.dailyBreakdown))
    ].map((day) => [day.date, day]));
    const olderActiveOverrideCount = olderGroups.reduce((total, group) => total + getWeekdayKeys(group.startDate, group.endDate)
        .filter((date) => activeDates.has(date)).length, 0);
    ...
    return (_jsxs("section", { className: "supervision-kpi-override-panel", "aria-label": `Overrides de EMRT para ${props.metric.label}`, children: [_jsxs("header", { children: [_jsx("strong", { children: "Excluir dia por override de EMRT" }), _jsx("span", { children: "El dia no contara como cumplido ni incumplido." })] }), _jsx("div", { className: "supervision-kpi-override-weeks", children: visibleGroups.map((group, index) => (_jsxs("div", { className: "supervision-kpi-override-week", id: index === 2 ? olderWeeksId : undefined, children: [_jsx("strong", { children: getGroupLabel(group, index) }), _jsx("div", { children: getWeekdayKeys(group.startDate, group.endDate).map((date) => {
                            ...
                            }) })] }, group.key))) }), olderGroups.length > 0 ? (_jsx("div", { className: "supervision-kpi-override-history-toggle", children: _jsxs("button", { type: "button", className: "secondary-button", "aria-controls": olderWeeksId, "aria-expanded": showOlderWeeks, onClick: () => setShowOlderWeeks((current) => !current), children: [_jsx("span", { children: showOlderWeeks ? "Ocultar semanas anteriores" : `Ver ${olderGroups.length} semanas anteriores` }), _jsxs("strong", { children: [olderActiveOverrideCount, " ", olderActiveOverrideCount === 1 ? "override activo" : "overrides activos"] })] }) })) : null] }));
}
