...
function getKpiOverrideKey(userId, metricId, date) {
    return `${userId}:${metricId}:${date}`;
}
function TaskUserRow(props) {
    const { user, kpiAlerts, kpiWeekReference, kpiOverrides, muted = false, saving, savingKpiOverrideKey, onToggleObserved, onToggleKpiOverride } = props;
...
function TaskOverviewPanel(props) {
    const { overview, kpiAlertsByUser, kpiWeekReference, kpiOverrides, savingObservedUserId, savingKpiOverrideKey, onToggleObserved, onToggleKpiOverride } = props;
...
function KpiMetricRow({ metric, periods, weekReference, view, showCommissionRelease, commissionRequirement, userId, kpiOverrides, savingKpiOverrideKey, onToggleKpiOverride }) {
...
function KpiOverrideControls(props) {
    ...
    const activeDates = new Set(props.overrides
    ...
    const dailyByDate = new Map(props.periods.flatMap((period) => period.metrics
        .filter((metric) => metric.id === props.metric.id)
        .flatMap((metric) => metric.dailyBreakdown)).map((day) => [day.date, day]));
    return (_jsxs("section", { className: "supervision-kpi-override-panel", "aria-label": `Overrides de EMRT para ${props.metric.label}`, children: [_jsxs("header", { children: [_jsx("strong", { children: "Excluir dia por override de EMRT" }), _jsx("span", { children: "El dia no contara como cumplido ni incumplido." })] }), _jsx("div", { className: "supervision-kpi-override-weeks", children: groups.map((group) => (_jsxs("div", { className: "supervision-kpi-override-week", children: [_jsx("strong", { children: group.label }), _jsx("div", { children: getWeekdayKeys(group.startDate, group.endDate).map((date) => {
                                ...
                                return (_jsxs("label", { className: `supervision-kpi-override-day ${checked ? "is-excluded" : ""} ${unavailable ? "is-unavailable" : ""}`, title: unavailable ? day?.actualLabel : undefined, children: [_jsx("input", { type: "checkbox", checked: checked, disabled: unavailable || Boolean(props.savingKey), onChange: (event) => props.onToggle(props.userId, props.metric.id, date, event.currentTarget.checked) }), _jsx("span", { children: formatOverrideDay(date) })] }, date));
                            }) })] }, group.label))) })] }));
}
...
export function GeneralSupervisionPage() {
    ...
    async function handleToggleKpiOverride(userId, metricId, date, isExcluded) {
        const key = getKpiOverrideKey(userId, metricId, date);
        setSavingKpiOverrideKey(key);
        setErrorMessage("");
        try {
            await apiPatch("/general-supervision/kpi-overrides", {
                userId,
                metricId,
                date,
                isExcluded
            });
            const reloaded = await apiGet("/general-supervision/overview");
            setClientKpiPeriods(null);
